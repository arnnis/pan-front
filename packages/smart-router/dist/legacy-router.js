...
chunkMWQ5Y6WM_js.__export(legacyRouter_exports, {
  ...
  getStableSwapPairs: () => getStableSwapPairs,
...
async function getPairs(currencyPairs, { provider, chainId }) {
  const tokens = currencyPairs.map(([currencyA, currencyB]) => [
    chunkMWQ5Y6WM_js.wrappedCurrency(currencyA, chainId),
    chunkMWQ5Y6WM_js.wrappedCurrency(currencyB, chainId)
  ]);
...
async function getStableSwapOutputAmount(pair, inputAmount, { provider }) {
  const wrappedInputAmount = chunkMWQ5Y6WM_js.wrappedCurrencyAmount(inputAmount, inputAmount.currency.chainId);
...
function getStableSwapFeeCall(pair, inputAmount) {
  ...
  const wrappedInputAmount = chunkMWQ5Y6WM_js.wrappedCurrencyAmount(inputAmount, chainId);
  if (!wrappedInputAmount) {
    throw new Error(`No wrapped token amount found for input amount: ${inputAmount.currency.name}`);
  }
  ...
  return {
    ...
    address: pair.infoStableSwapAddress || chunkMWQ5Y6WM_js.STABLE_SWAP_INFO_ADDRESS[chainId],
...
async function getAllCommonPairs(currencyA, currencyB, { provider }) {
  ...
  const additionalA = tokenA ? chunkMWQ5Y6WM_js.ADDITIONAL_BASES[chainId]?.[tokenA.address] ?? [] : [];
  const additionalB = tokenB ? chunkMWQ5Y6WM_js.ADDITIONAL_BASES[chainId]?.[tokenB.address] ?? [] : [];
...
function createStableSwapPair(pair, stableSwapAddress = "0x", lpAddress = "0x", infoStableSwapAddress = "0x", stableLpFee = 0, stableTotalFee = 0, stableLpFeeRateOfTotalFee = 0) {
  return {
    ...pair,
    stableSwapAddress,
    lpAddress,
...
async function getStableSwapPairs(chainId) {
  const pools = await stableSwapSdk.getStableSwapPools(chainId);
  return pools.map(
    ({
...
async function getBestTradeFromStablePools(amount, output, { maxHops, provider }) {
  ...
  const pairs = await getStableSwapPairs(chainId) || [];
...
function createGetBestTradeFromV2(tradeType) {
  ...
  return async function bestTradeFromV2(amountIn, output, options) {
    ...
    for (let i = 1; i <= maxHops; i++) {
      ...
      if (sdk.isTradeBetter(bestTradeSoFar, currentTrade, chunkMWQ5Y6WM_js.BETTER_TRADE_LESS_HOPS_THRESHOLD)) {
