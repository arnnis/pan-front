...
var pcsxSdk = require('@pancakeswap/pcsx-sdk');
...
function getRequestUrl(params) {
  ...
  const key = getCurrencyListKey(infoList.filter((c) => !chains.isTestnetChainId(c.chainId)));
  if (!key) {
    return void 0;
  }
...
async function getCurrencyUsdPrice(currencyParams, options) {
  if (!currencyParams || chains.isTestnetChainId(currencyParams.chainId)) {
    return 0;
  }
...
function getPoolType(type) {
  return smartRouter.PoolType[type];
}
function getPoolTypeKey(poolType) {
  for (const [key, value] of Object.entries(smartRouter.PoolType)) {
    if (value === poolType) {
      return key;
    }
  }
...
function getTradeTypeKey(tradeType) {
  for (const [key, value] of Object.entries(swapSdkCore.TradeType)) {
    if (value === tradeType) {
      return key;
    }
  }
...
function getRequestBody({ amount, quoteCurrency, tradeType, amm, x, slippage }) {
  const [currencyIn, currencyOut] = tradeType === swapSdkCore.TradeType.EXACT_INPUT ? [amount.currency, quoteCurrency] : [quoteCurrency, amount.currency];
  const configs = [];
  if (amm) {
    configs.push({
...
function parseQuoteResponse(res, {
  ...
  if (bestOrder.type === "PCS_CLASSIC" /* PCS_CLASSIC */) {
    return {
      ...
      trade: smartRouter.V4Router.Transformer.parseTrade(chainId, bestOrder.order)
    };
  }
  if (bestOrder.type === "DUTCH_LIMIT" /* DUTCH_LIMIT */) {
    const order = pcsxSdk.ExclusiveDutchOrder.fromJSON(bestOrder.order.orderInfo, chainId);
    const otherAmmTrade = allPossibleOrders.find((o) => o.type === "PCS_CLASSIC" /* PCS_CLASSIC */);
    return {
      type: "DUTCH_LIMIT" /* DUTCH_LIMIT */,
      ammTrade: otherAmmTrade ? smartRouter.V4Router.Transformer.parseTrade(chainId, otherAmmTrade.order) : void 0,
      trade: pcsxSdk.createExclusiveDutchOrderTrade({
        currencyIn,
...
function parseAMMPriceResponse(chainId, res) {
  ...
  const trade = smartRouter.V4Router.Transformer.parseTrade(chainId, rest);
  return {
    ...trade,
...
exports.getCurrencyKey = getCurrencyKey;
exports.getCurrencyListKey = getCurrencyListKey;
exports.getCurrencyListUsdPrice = getCurrencyListUsdPrice;
exports.getCurrencyUsdPrice = getCurrencyUsdPrice;
exports.getNativeTokenPrices = getNativeTokenPrices;
exports.getPoolType = getPoolType;
exports.getPoolTypeKey = getPoolTypeKey;
exports.getRequestBody = getRequestBody;
exports.getTokenPrices = getTokenPrices;
exports.parseAMMPriceResponse = parseAMMPriceResponse;
exports.parseQuoteResponse = parseQuoteResponse;
